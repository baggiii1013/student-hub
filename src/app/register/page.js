  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (loginResponse.accessToken) {
        const tokenPayload = JSON.parse(atob(loginResponse.accessToken.split('.')[1]));
      ...
      <div className="relative z-10 w-full max-w-md px-3 sm:px-4 md:px-6">
        <div className={`transform transition-all duration-1000 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <div className="text-center mb-4 sm:mb-6 md:mb-8">
            ...
            <p className="text-gray-300 text-xs sm:text-sm md:text-base">Join our community! Create your account to get started.</p>
          </div>
